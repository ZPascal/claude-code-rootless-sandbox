...
enabled: true
conditions:
  - keywords: ["sandbox", "isolated", "rootless", "container", "docker", "podman", "docker-sandbox"]
  - phrases:
      - "run this in sandbox"
      - "run this isolated"
      - "start the sandbox"
...
This skill is **enabled by default**. You can disable it in two ways:

### Option 1: Environment Variable (Temporary)

Disable the skill for a single session:

```bash
export CLAUDE_DISABLE_DOCKER_SANDBOX=1
# Claude Code will not recognize sandbox requests in this session
```
...
# or
export CLAUDE_DISABLE_DOCKER_SANDBOX=0
```

### Option 2: Remove Skill File (Permanent)

Permanently disable the skill:
...
Disable in your Claude Code settings:
...
**Questions?** See the [main repository](https://github.com/ZPascal/claude-code-rootless-sandbox) or open an issue.
